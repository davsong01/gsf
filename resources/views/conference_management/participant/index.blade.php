				...
				<div class="card" style="height: 250px; background-color: {{ $payment->edition->id == $edition->id ? 'green':'gray;'}}">
					<a href="{{ route('conferencemanagement.show', ['conferencemanagement'=>$payment->id, 'edition'=>$payment->conference_edition_id]) }}">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <small style="color:{{ $payment->edition->id == $edition->id ? 'white':'black'}}">{{ formatDates($payment->edition->start_date, $payment->edition->end_date) }}<br> <span style="color:yellow">{{ $payment->level }}</span>
                            @if($payment->registration_user_type != 'Participant')
                            <span>({{ucfirst($payment->registration_user_type)}})</span>
                            @endif
                            </small>
                        </div>
                        ...
                        <div class="card-content">
                            <div class="card-body">
                                ...
                                <span style="color:yellow"><strong>Plan Name:</strong> <span style="color:white">{{ $payment->conferenceplan->title }}</span> <br>
