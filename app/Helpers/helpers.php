...
if (!function_exists('coursesOfStudy')) {
...
if (!function_exists('stakeholderSetting')) {
    function stakeholderSetting(string|array $key = '*')
    {
        // Return all settings as key => value
        if ($key === '*') {
            return StakeholderSetting::pluck('value', 'key')->toArray();
        }

        // Return multiple keys
        if (is_array($key)) {
            return StakeholderSetting::whereIn('key', $key)
                ->pluck('value', 'key')
                ->toArray();
        }

        // Return single key
        return StakeholderSetting::where('key', $key)
            ->value('value');
    }
}
...
if (!function_exists('resolveAwardPermissions')) {
    function resolveAwardPermissions(Award $award, $user = null): object
    {
        ...
        $isAdminDetails        = isAdmin();
        $isAdmin = $isAdminDetails['status'];
        $userRole = $isAdminDetails['userRole'];
